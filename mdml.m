...
% Initialize the metric learning variables
mu = 1;
V = eye(N);
E = ones(N, 1);
...
for t = 1:T  
    if useProgressBar == 1
        if mod(t, 100) == 0
            % Update the waitbar
            waitbar(t/T, h, sprintf('%3.0f%% done', t/T*100));
        end
    end
...
fprintf('Num pairs of instances seen = %d, Num updates = %d.\n', T, numUpdates);
